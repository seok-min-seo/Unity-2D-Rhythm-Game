...
public class NoteController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //첫번쨰 줄에 적힌 곡 이름을 읽습니다.
        musicTitle = reader.ReadLine();
        //두번쨰 줄에 적힌 아티스트 이름을 읽습니다.
        musicArtist = reader.ReadLine();
        //세번째 줄에 적힌 비트 정보(BPM, Divider, 시작 시간)를 읽습니다.
        ...
        while((line = reader.ReadLine()) != null)
        {
            Note note = new Note(
                ...
                //공백을 기준으로 나눠서 첫번쨰는 +1된 노트자리를 반환하고(노트자리는 4개)
                Convert.ToInt32(line.Split(' ')[1]) 
                //뒤에는 몇초에 시작할지를 반환
                );
            notes.Add(note); //미리 만들어둔 노트 리스트에 모두 담겨서 떨어지게됨
        }
